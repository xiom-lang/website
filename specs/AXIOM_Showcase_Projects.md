...
> Both stress what AXIOM was built for — no-GC determinism, contracts as
...
**AxiomDB first. AxiomVDB second.** A key-value store with B-tree forces
implementation of every systems primitive AXIOM needs: heap allocation, file
I/O, byte-level memory management, crash recovery. AxiomVDB skips half of
these because it can lean on brute-force k-NN for correctness. Building
AxiomDB first gives AxiomVDB a battle-tested storage engine to build on.
...
Building AxiomDB first means the storage engine is written once and reused.
...
- Target: ~2,000 lines of AXIOM

### Phase B: Transactions (post-AxiomVDB)

Only after AxiomVDB exercises the storage engine from a different domain:
...
4. **STOP.** Verify. Build AxiomVDB on this foundation.
...
- Built on AxiomDB's storage engine (page pool, WAL)
...
for the wire layer later; don't write AXIOM's own networking stack.
...
Python, and other languages via FFI. This doubles as AXIOM's first real test
of the *outbound* FFI story — AXIOM exposing itself to other languages, not
...
| Builds on | (self) | AxiomDB storage engine |
...
- AxiomVDB directly integrates with Kyberon's MNEME memory subsystem

---

*AXIOM Showcase Projects — Version 1.0. Decisions made 2026-06-30.*
